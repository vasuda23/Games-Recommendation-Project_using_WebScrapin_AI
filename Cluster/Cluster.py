...
tweets = []
for line in open('uniqueData.txt.txt').readlines():
    tweets.append(line)
...
vocab = {t: freq for t, freq in vocab.items() if freq > 20}
# Generate an id (starting from 0) for each term in vocab
vocab = {t: idx for idx, (t, freq) in enumerate(vocab.items())}

# Generate X
X = []
for tweet_text in tweets:
    x = [0] * len(vocab)
    terms = [t for t in tweet_text.split() if len(t) > 2]
    for t in terms:
        if vocab.has_key(t):
            x[vocab[t]] += 1
    X.append(x)
...
# K-means clustering
km = KMeans(n_clusters = 10, n_init = 100) # try 100 different initial centroids
km.fit(X)
